function executeCommands(commands) {
  ...
  const comms = {
    MOV: (values) => {
      ...
      arr[0].startsWith('V')
        ? (output[arr[1][2]] = output[arr[0][2]])
        : (output[arr[1][2]] = +arr[0]);
    },
    ADD: (values) => {
      let arr = values.split(',');
      output[arr[0][2]] += output[arr[1][2]];
      output[arr[0][2]] > 255
        ? (output[arr[0][2]] = output[arr[0][2]] - 256)
        : 0;
    },
    DEC: (values) => {
      output[values[2]] === 0 ? (output[values[2]] = 255) : output[values[2]]--;
    },
    INC: (values) => {
      output[values[2]] === 255 ? (output[values[2]] = 0) : output[values[2]]++;
    },
    JMP: (i) => {
      let jmp = commands.indexOf(`JMP ${i}`);
      output[0] !== 0
        ? commands.slice(i, jmp + 1).forEach((command) => {
            let comm = command.split(' ')[0];
            let values = command.split(' ')[1];
            comms[comm](values);
          })
        : 0;
    }
