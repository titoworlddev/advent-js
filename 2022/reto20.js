function howManyReindeers(reindeerTypes, gifts) {
  return gifts.map(({country, weight}) => {
    ...
    let reindeers = reindeerTypes
      ...
      .filter((x) => x[1] < max)
      .sort((a, b) => b[1] - a[1]); // Menor a Mayor
    ...
    reindeers.forEach((_, i) => {
      let sliced = reindeers.slice(i, reindeers.length)
      let sum = sliced.reduce((total, e) => total + e[1], 0);
      sliced.forEach((_, idx) => res[idx + i].num += Math.floor(max / sum));
      max %= sum; // max = max % sum;
    });
